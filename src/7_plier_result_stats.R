...
plier_results <- readRDS('../output/filtered_plier.rds')
...
png('../output/filtered_lv_per_pathway_hist.png', width=720, height=720)
...
png('../output/filtered_gene_per_pathway_hist.png', width=720, height=720)
hist(colSums(C)[colSums(C) < 200], main='Distribution of genes per pathway (truncated at 200)',
     xlab='Genes per pathway', breaks=50)
dev.off()

png('../output/filtered_percent_genes_used_hist.png', width=720, height=720)
...
write.table(Z, file='../output/filtered_Z.tsv', sep='\t')
write.table(U, file='../output/filtered_U.tsv', sep='\t')
fileConn<-file('../output/filtered_lambda.txt')
